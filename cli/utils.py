...
def detect_asset_type(ticker: str, *, force_crypto: bool = False) -> AssetType:
    ...
    When ``force_crypto`` is True (crypto CLI / crypto mode), bare bases like
    BTC, ETH, SOL, HYPE are also treated as crypto without requiring a
    Yahoo-style ``-USD`` suffix.
    """
    if force_crypto:
        from tradingagents.crypto.instruments import is_standalone_crypto_symbol

        if is_standalone_crypto_symbol(ticker):
    ...
    if force_crypto:
        ...
        if is_standalone_crypto_symbol(canonical):
            return AssetType.CRYPTO
    return AssetType.STOCK


CRYPTO_ANALYST_ORDER = [
    ("Market Structure Analyst", AnalystType.MARKET),
    ("Derivatives Analyst", AnalystType.DERIVATIVES),
    ("Sentiment Analyst", AnalystType.SOCIAL),
    ("Catalyst Analyst", AnalystType.CATALYST),
    ("Regime Analyst", AnalystType.REGIME),
]
...
def filter_analysts_for_asset_type(
    ...
    if asset_type != AssetType.CRYPTO:
        return analysts
    # Legacy interactive stock-style selection: drop fundamentals for crypto.
    # Dedicated crypto analyze path uses CRYPTO_ANALYST_ORDER instead.
    return [
        analyst
...
def select_analysts(asset_type: AssetType = AssetType.STOCK) -> list[AnalystType]:
    ...
    order = CRYPTO_ANALYST_ORDER if asset_type == AssetType.CRYPTO else ANALYST_ORDER
    available_analysts = filter_analysts_for_asset_type(
        [value for _, value in order],
        asset_type,
    )
