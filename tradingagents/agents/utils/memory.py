...
class TradingMemoryLog:
    ...
    def store_decision(
        ...
        *,
        ...
        Optional ``asset_type`` / ``trade_proposal`` enrich crypto entries with a
        ``CRYPTO_META`` JSON block that crypto resolvers understand; stock
        parsers ignore the HTML comment.
        """
        ...
        if self._log_path.exists():
            ...
            for line in raw.splitlines():
                if line.startswith(f"[{trade_date} | {ticker} |") and line.endswith("| pending]"):
                    return
        decision_body = final_trade_decision or ""
        if asset_type == "crypto" or trade_proposal is not None:
            from tradingagents.agents.utils.crypto_memory import build_crypto_meta_block

            # Avoid duplicating an existing meta block
            if "CRYPTO_META" not in decision_body:
                decision_body = decision_body.rstrip() + build_crypto_meta_block(
                    asset_type=asset_type or "crypto",
                    trade_proposal=trade_proposal,
                )
        rating = parse_rating(decision_body)
        tag = f"[{trade_date} | {ticker} | {rating} | pending]"
        entry = f"{tag}\n\nDECISION:\n{decision_body}{self._SEPARATOR}"
